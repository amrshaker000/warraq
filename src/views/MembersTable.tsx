...
const MembersTable: React.FC = () => {
  ...
  const performAdvancedSearch = useCallback((query: string, members: Member[]): Member[] => {
  ...
  const filteredMembers = useMemo(() => {
    ...
    filtered = filtered.filter((member: Member) => {
      ...
      const matchesFinancialSupport = filters.financialSupport === 'all' || member.financialSupport === filters.financialSupport;
      const matchesMembershipType = filters.membershipType === 'all' || member.membershipType === filters.membershipType;
      const matchesPartyUnit = filters.partyUnit === 'all' || (member.partyUnit || '') === filters.partyUnit;

      return matchesGender && matchesStatus && matchesFinancialSupport && matchesMembershipType && matchesPartyUnit;
    });
  ...
  const stats = useMemo(() => {
    ...
    const paid = members.filter((m: Member) => m.financialSupport === 'paid').length;
  ...
  // Event handlers
  const handleFilterChange = useCallback((key: keyof MemberFilters, value: string) => {
  ...
  const handleDeleteMember = async (member: Member) => {
    if (window.confirm(t('members.deleteConfirmation', { name: member.fullName }))) {
  ...
  const handleBulkDelete = async () => {
    ...
    const confirmMessage = t('members.bulkDeleteConfirm', { count: selectedMembers.size });
    ...
    if (window.confirm(confirmMessage)) {
      ...
      try {
        ...
        addToast({
          title: t('common.success'),
          message: t('members.bulkDeleteSuccess', { count: selectedMembers.size }),
          type: 'success'
        });
  ...
  const handleBulkExport = async () => {
    ...
    setIsExporting(true);
    try {
      const selectedMembersData = members.filter((m: Member) => selectedMembers.has(m.id));
      await ExcelService.exportToExcel(selectedMembersData);

      addToast({
        title: t('common.success'),
        message: t('members.exportSuccess', { count: selectedMembersData.length }),
        type: 'success'
      });
    } catch {
      addToast({
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-3 lg:gap-4">
                <div className="relative">
                  <Input
                    ...
                    className={`w-full transition-all duration-200 ${
                      isSearching ? 'ring-2 ring-primary-500 border-primary-500' : ''
                    }`}
                  />
                ...
                <Select
                  ...
                  onChange={(e) => handleFilterChange('financialSupport', e.target.value)}
                ...
                <Select
                  ...
                  onChange={(e) => handleFilterChange('partyUnit', e.target.value)}
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium text-blue-700 dark:text-blue-300">
                    {t('common.selectedMembers', { count: selectedMembers.size })}
                  </span>
                </div>
                ...
                <div className="max-w-md mx-auto text-center">
                  ...
                  <div className="space-y-3 mb-8 flex flex-col items-center">
                    <div className="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400">
                      ...
                      <span>{t('members.emptyState.features.addMembers')}</span>
                    </div>
                    <div className="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400">
                      <div className="w-2 h-2 bg-blue-500 rounded-full"></div>
                      <span>{t('members.emptyState.features.manageData')}</span>
                    </div>
                    <div className="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400">
                      <div className="w-2 h-2 bg-purple-500 rounded-full"></div>
                      <span>{t('members.emptyState.features.advancedSearch')}</span>
                    </div>
                  </div>
              ...
              <>
                ...
                <div className="bg-gray-50 dark:bg-gray-800 px-4 py-3 border-b border-gray-200 dark:border-gray-700">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-sm text-gray-600 dark:text-gray-400">{t('common.view')}:</span>
                      <Button
                            ...
                            <div className="flex items-start gap-3 flex-1 min-w-0">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-sm text-gray-500 dark:text-gray-400">
                                  {t('members.membershipNumber')}: <span className="ltr">{member.membershipNumber}</span>
                                </p>
                              </div>
                            </div>
                          ...
                          <div className="space-y-2 sm:space-y-3">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 text-sm">
                              <div className="min-w-0">
                                <span className="text-gray-500 dark:text-gray-400 block text-xs font-medium">{t('members.nationalId')}:</span>
                                <p className="font-medium text-gray-900 dark:text-white truncate ltr">{member.nationalId}</p>
                              </div>
                              <div className="min-w-0">
                                <span className="text-gray-500 dark:text-gray-400 block text-xs font-medium">{t('members.gender')}:</span>
                                <p className="font-medium text-gray-900 dark:text-white">{t(`common.${member.gender}`)}</p>
                              </div>
                              <div className="min-w-0">
                                <span className="text-gray-500 dark:text-gray-400 block text-xs font-medium">{t('members.phoneNumber')}:</span>
                                <p className="font-medium text-gray-900 dark:text-white truncate ltr">{member.phoneNumber}</p>
                              </div>
                              <div className="min-w-0">
                                <span className="text-gray-500 dark:text-gray-400 block text-xs font-medium">{t('members.partyUnit')}:</span>
                                <p className="font-medium text-gray-900 dark:text-white truncate">{member.partyUnit || '-'}</p>
                              </div>
                              <div className="min-w-0">
                                <span className="text-gray-500 dark:text-gray-400 block text-xs font-medium">{t('members.age')}:</span>
                                <p className="font-medium text-gray-900 dark:text-white ltr">{member.age}</p>
                              </div>
                            </div>
                            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 pt-2 border-t border-gray-100 dark:border-gray-700">
                              <div className="flex items-center gap-2">
                                <span className="text-gray-500 dark:text-gray-400 text-xs font-medium">{t('members.status')}:</span>
                              ...
                              <div className="flex items-center gap-2">
                                <span className="text-gray-500 dark:text-gray-400 text-xs font-medium">{t('members.membershipType')}:</span>
                                <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300">
                                  {t(`members.memberTypes.${member.membershipType}`)}
                                </span>
                              </div>
                            </div>
                          </div>
                  ...
                  <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between w-full">
                    <div>
                      <p className="text-sm text-gray-700 dark:text-gray-300 font-arabic">
                        {t('members.pagination.showing', {
                          ...
                          to: Math.min(currentPage * itemsPerPage, filteredMembers.length),
                          total: filteredMembers.length
                        })}
                      </p>
                    </div>
                    <div>
                      <nav className="relative z-0 inline-flex rounded-md shadow-sm space-x-reverse space-x-1 rtl:space-x-reverse">
                        ...
                        >
                          <ChevronRight className="h-4 w-4 ml-1" />
                          <span className="mr-2 font-arabic">{t('common.previous')}</span>
                        </Button>
                        ...
                        {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                          ...
                          if ((i === 1 && currentPage > 3) || (i === 3 && currentPage < totalPages - 2)) {
                            return <span key={i} className="px-3 py-1 text-sm text-gray-500">...</span>;
                          }
                        ...
                        <Button
                          ...
                          className="flex items-center"
                        >
                          <span className="ml-2 font-arabic">{t('common.next')}</span>
                          <ChevronLeft className="h-4 w-4 mr-1" />
                        </Button>
                      </nav>
                    </div>
