...
const Toast: React.FC<ToastProps> = ({
  ...
  const colors = {
    success: 'bg-green-50/95 border-green-200 text-green-900 dark:bg-green-950/95 dark:border-green-800 dark:text-green-100',
    error: 'bg-red-50/95 border-red-200 text-red-900 dark:bg-red-950/95 dark:border-red-800 dark:text-red-100',
    warning: 'bg-amber-50/95 border-amber-200 text-amber-900 dark:bg-amber-950/95 dark:border-amber-800 dark:text-amber-100',
    ...
    >
      <div className="p-4">
        <div className="flex items-start gap-4">
          <div className={clsx('flex-shrink-0 p-2 rounded-full', circleBgColors[type])}>
            {isIndefinite ? (
              <Loader2 className={clsx('h-5 w-5 animate-spin', iconColors[type])} />
            ) : (
              <Icon className={clsx('h-5 w-5', iconColors[type])} />
            )}
          </div>
          <div className={clsx('min-w-0 flex-auto flex flex-col gap-1')}>
            <h4 className="text-sm font-semibold break-words leading-tight">{String(title)}</h4>
            {message && (
              <p className="text-sm leading-relaxed opacity-90 break-words whitespace-pre-wrap">{String(message)}</p>
            )}
          </div>
