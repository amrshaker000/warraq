...
export class ExcelService {
  ...
  static async exportToExcel(members: Member[], filename: string = 'أعضاء_الحزب.xlsx'): Promise<void> {
    this.exportMembers(members, filename);
  }

  static exportMembers(members: Member[], filename: string = 'أعضاء_الحزب.xlsx'): void {
    try {
      ...
      const exportData = members.map(member => ({
        ...
        'الوظيفة': member.job,
        'حالة العضو': member.status === 'active' ? 'نشط' : member.status === 'inactive' ? 'غير نشط' : 'معلق',
        'نوع العضوية': this.getMembershipTypeText(member.membershipType),
        'الدعم المالي': member.financialSupport === 'paid' ? 'مدفوع' : 'غير مدفوع',
        'تاريخ التسجيل': new Date(member.registrationDate).toLocaleDateString('ar-EG'),
      }));

      const worksheet = XLSX.utils.json_to_sheet(exportData);
      worksheet['!cols'] = [
        { wch: 20 }, { wch: 15 }, { wch: 8 }, { wch: 15 }, { wch: 15 },
        { wch: 25 }, { wch: 12 }, { wch: 6 }, { wch: 30 }, { wch: 20 },
  ...
  static async importMembers(file: File): Promise<Member[]> {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e: ProgressEvent<FileReader>) => {
        try {
          const data = new Uint8Array((e.target?.result as ArrayBufferLike) || new ArrayBuffer(0));
          ...
          reject(new Error(`فشل في استيراد البيانات: ${error instanceof Error ? error.message : 'خطأ غير معروف'}`));
        }
      };
  ...
  private static parseMemberFromExcel(row: Record<string, unknown>, index: number): Member {
    const fieldMappings = {
